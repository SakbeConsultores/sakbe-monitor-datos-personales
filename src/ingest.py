"""
Script principal de ingesta del Monitor Regulatorio de Protección de
Datos Personales.

Para cada fuente definida en config/feeds.yaml:
    1. Salta la fuente si enabled es false o si source es "pending".
    2. Lee los items (vía RSS o scraper, según el config).
    3. Filtra los que ya están en Notion (dedup por URL).
    4. Inserta los items nuevos en la base de Notion.
    5. Exporta docs/data/monitor.json para el dashboard.
...
DIFERENCIAS respecto al ingest del Monitor de Reguladores Financieros:
    - Propaga el campo "region" (LatAm / Europa) al item y a Notion.
      Es lo que alimenta las pestañas del dashboard.
    - Respeta los campos "enabled" y source: "pending", para que el
      pipeline corra sin tronar mientras las 40 fuentes se van
      verificando una por una.
    - El resumen final reporta cuántas fuentes quedan pendientes.
"""
...
def load_config(config_path: Path) -> dict:
    """Lee y parsea el YAML de configuración de fuentes."""
    with open(config_path, "r", encoding="utf-8") as f:
        return yaml.safe_load(f)


def is_active(feed_cfg: dict) -> bool:
    """
    Decide si una fuente se procesa en esta corrida.

    Una fuente está activa cuando enabled es true Y el source es uno
    que sabemos leer. Mientras el feed no esté verificado, el bloque
    queda con enabled: false y source: pending, y aquí se salta sin
    generar error. Esto permite tener las 40 autoridades declaradas
    desde el día uno e ir habilitándolas conforme se confirma cada
    fuente, sin romper el pipeline.
    """
    if not feed_cfg.get("enabled", False):
        return False
    if feed_cfg.get("source") not in ("rss", "scraper"):
        return False
...
def fetch_feed_items(feed_cfg: dict) -> list:
    """
    Dispatcher: según el source de la fuente, llama a rss_parser
    ...
    if source == "scraper":
        ...
        if not module_name:
            log.error("Fuente con source=scraper pero sin scraper_module: %s", url)
            return []
        try:
...
def main():
    ...
    # Cargar configuración de fuentes y settings globales
    ...
    # Separamos activas de pendientes antes de arrancar, para que el log
    # diga con claridad qué parte del monitor está viva.
    active_feeds = [f for f in feeds if is_active(f)]
    pending_feeds = [f for f in feeds if not is_active(f)]

    log.info(
        "Fuentes declaradas: %d. Activas: %d. Pendientes de verificar: %d. "
        "Filtro de antigüedad: %d días.",
        len(feeds), len(active_feeds), len(pending_feeds), max_age_days,
    )
    ...
    if pending_feeds:
        log.info(
            "Pendientes: %s",
            ", ".join(
                f"{f.get('regulator', '?')} ({f.get('country', '?')})"
                for f in pending_feeds
            ),
        )

    if not active_feeds:
        log.warning(
            "Ninguna fuente activa. El ingest no va a insertar nada. "
            "Corre el workflow 'Descubrir feeds' y llena url + source + "
            "enabled en config/feeds.yaml."
        )
    ...
    for feed_cfg in active_feeds:
        ...
        log.info("--- [%s] %s (%s) - %s", region, regulator, country, feed_type)
    ...
    # -----------------------------------------------------------------
    # Export JSON para el dashboard
    # -----------------------------------------------------------------
    # Traemos todos los items con sus propiedades y los serializamos a
    # docs/data/monitor.json. GitHub Pages lo sirve públicamente y el
    # dashboard lo consume con un solo fetch, sin pasar por Notion.
    log.info("Generando export JSON para el dashboard...")
    all_items = notion.get_all_items()

    # Recortamos a la ventana de días que queremos mostrar.
    ...
    # El bloque "sources" le dice al dashboard qué autoridades están
    # declaradas y en qué estado, aunque todavía no hayan producido un
    # solo item. Así la pestaña muestra cobertura real en lugar de
    # aparentar que solo existen las autoridades que ya publicaron.
    sources = [
        {
    ...
    export = {
        ...
        "regions": settings.get("regions", ["LatAm", "Europa"]),
        "sources_total": len(feeds),
        "sources_active": len(active_feeds),
        "sources": sources,
        "items": display_items,
    }
